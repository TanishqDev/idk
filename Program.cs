...
" + " > Discord: dsc.gg/extatent" + @"       ╚═╝     ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═══╝╚═╝╚═╝  ╚═╝
        ...
        static void Main()
        {
            foreach (Process process in Process.GetProcesses()) { if (Process.GetCurrentProcess().ProcessName == process.ProcessName && Environment.ProcessId != process.Id) { Environment.Exit(0); } }
            Console.Title = "Phoenix Nuker";
            try
            {
                Console.ForegroundColor = Color.Yellow;
                if (!File.Exists(Path.GetTempPath() + "token.txt"))
                    File.Create(Path.GetTempPath() + "token.txt").Dispose();
                if (!File.Exists("multitokens.txt"))
                    File.Create("multitokens.txt").Dispose();
                var Token = File.ReadAllText(Path.GetTempPath() + "token.txt");
...
";
                    ...
                    case 37:
                        ...
                        Task.Run(() => { for (int i = 0; i < mcount; i++) { try { Guild.SendWebhookMessage(token, wid2, wtoken, message2); } catch { } } });
                        Options();
                        break;
                    case 38:
                        GuildID();
...
";
                    ...
                    case 47:
                        ...
                        ulong? cid8 = ulong.Parse(Console.ReadLine());
                        WriteLogo();
                        Console.Write("Message: ");
                        string msg2 = Console.ReadLine();
                        WriteLogo();
                        foreach (var token in tokenlist) Raid.DMUser(token, cid8, msg2);
                        Options();
                        break;
                    case 48:
                        MultiTokens();
...
";
                    ...
                    case 53:
                        MultiTokens();
                        WriteLogo();
                        Task.Run(() => { try { foreach (var token in tokenlist) { Raid.OnlineTokens(token); } } catch { } });
                        Options();
                        break;
                    case 54:
                        MultiTokens();
                        WriteLogo();
                        Console.Write("Guild ID: ");
                        ulong? gid5 = ulong.Parse(Console.ReadLine());
                        WriteLogo();
                        Console.Write("Voice Channel ID: ");
                        ulong? vid = ulong.Parse(Console.ReadLine());
                        WriteLogo();
                        Task.Run(() => { try { foreach (var token in tokenlist) { Raid.JoinVC(token, gid5, vid); } } catch { } });
                        Options();
                        break;
                    case 55:
                        foreach (var driver in Process.GetProcessesByName("chromedriver")) { driver.Kill(); }
                        Environment.Exit(0);
                        break;
                    case 56:
                        WriteLogo();
        ...
        static void Token()
        {
            if (string.IsNullOrEmpty(token))
            {
                try
                {
                    ...
                    File.WriteAllText(Path.GetTempPath() + "token.txt", token);
                    try { Request.Send("/users/@me", "GET", token); } catch { IsBot = true; }
                    Console.Clear();
                }
                catch { }
        ...
        static void LoginToAccount()
        {
            ...
            {
                Console.WriteLine("This feature supports only the Google Chrome browser.");
                Sleep(Wait.Long);
                Options();
            }
            catch (Exception e)
